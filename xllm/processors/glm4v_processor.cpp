...
std::optional<Size> smart_resize(int num_frames,
  ...
  if(num_frames < temporal_factor){
    LOG(ERROR) << "t:{num_frames} must be larger than temporal_factor:{temporal_factor}";
    return std::nullopt;
  }
  ...
  int t_bar =  static_cast<int>(std::round(num_frames / static_cast<double>(temporal_factor))) *
      temporal_factor; 
  int h_bar =
  ...
  if (t_bar * h_bar * w_bar > max_pixels) {
    double beta = std::sqrt((num_frames*height * width) / static_cast<double>(max_pixels));
    ...
    double beta = std::sqrt(min_pixels / static_cast<double>(height * width*num_frames));
    h_bar = static_cast<int>(
...
bool Glm4VImageProcessor::process_image(
  ...
  if(shape[0] % temporal_patch_size_ !=0){
    auto repeats =
        patches[-1].unsqueeze(0).repeat({temporal_patch_size_-(shape[0] % temporal_patch_size_), 1, 1, 1});
...
bool Glm4VImageProcessor::process_videos(std::vector<torch::Tensor> videos,
...
bool Glm4VImageProcessor::process_video(
  ...
  //TODO: wait for wangziyue‘s sample_frames implementation.
  if (do_sample_frame_) {
  ...
  //if (pad_t > 0) {
  auto last =
      out_video.index({time_len - 1}).unsqueeze(0).repeat({temporal_patch_size_-1, 1, 1, 1});
