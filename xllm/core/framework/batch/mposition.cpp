...
torch::Tensor MPositionHelper::get_positions() {
  ...
  if (seq_.kv_state().kv_cache_tokens_num() < seq_.num_prompt_tokens()) {
    ...
    if (absl::StartsWith(args_.model_type(), "qwen2_5_vl")) {
      res = get_positions_p(image_grid_thw, video_grid_thw, second_per_grid_ts);
    } else if (absl::StartsWith(args_.model_type(), "glm4v")) {
...
std::tuple<torch::Tensor, int> MPositionHelper::get_positions_qwen3(
  ...
  // 找 <vision_start> 后面的那个 token 来统计 image/video 段数
  ...
  llm_pos_ids_list.reserve(image_nums + video_nums + 2);
  ...
  for (int i = 0; i < image_nums + video_nums; ++i) {
    int ed_image = (int)input_tokens.size() + 1;
    int ed_video = (int)input_tokens.size() + 1;

    if (remain_images > 0) {
      auto it = std::find(
          input_tokens.begin() + st, input_tokens.end(), image_token_id);
      if (it != input_tokens.end())
        ed_image = (int)std::distance(input_tokens.begin(), it);
    }
    if (remain_videos > 0) {
      auto it = std::find(
  ...
  if (st < (int)input_tokens.size()) {
    ...
    if (!llm_pos_ids_list.empty())
      st_idx = llm_pos_ids_list.back().max().item<int>() + 1;
    int text_len = (int)input_tokens.size() - st;
    auto text_pos =
        torch::arange(text_len, torch::kInt32).view({1, -1}).expand({3, -1}) +
        st_idx;
    llm_pos_ids_list.push_back(text_pos);
