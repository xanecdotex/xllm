...
class Qwen3_VisionTransformerImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor rot_pos_emb(torch::Tensor grid_thw) {
    ...
    LOG(INFO) << "rotary_pos_emb_full: " << rotary_pos_emb_full.sizes()
              << " ==== " << rotary_pos_emb_full;
    // LOG(INFO) << "pos_ids: " << pos_ids.sizes() << " ==== " << pos_ids;
  ...
  std::tuple<torch::Tensor, std::vector<torch::Tensor>> forward(
    ...
    // LOG(INFO) << "rotary_pos_emb: " << rotary_pos_emb.sizes() <<
    // rotary_pos_emb;
    LOG(INFO) << "grid_thw: " << grid_thw.sizes() << grid_thw;
    // compute cu_seqlens
    ...
    LOG(INFO) << "cu_seqlens: " << cu_seqlens.sizes() << cu_seqlens;
    ...
    // // std::vector<torch::Tensor> all_features = {hidden_states};
    // // all_features.insert(all_features.end(),
    // deepstack_feature_lists_.begin(), deepstack_feature_lists_.end()); return
